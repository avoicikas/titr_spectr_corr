...
def plot_psds():
    ...
    for ifile in failai:
        ...
        for group, datt in df.groupby(by=['subjname']):
            ...
            im=mne.viz.plot_topomap(profile_values.igf,pos,axes=ax[1][0],cmap=cm.viridis,contours=0,vmin=35)
            plt.colorbar(im[0],ax=ax[1][0])
            im=mne.viz.plot_topomap(profile_values.igf_value,pos,cmap=cm.viridis, contours=0, axes=ax[1][1])
